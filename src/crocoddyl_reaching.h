...
public:
    CrocoddylReaching(pin::Model _model_pin, CrocoddylConfig _config)
    {
        ...
        Eigen::Matrix<double, 14, 1> diag_x_reg_terminal;
        diag_x_reg_terminal << diag_q_reg_terminal, diag_v_reg_terminal;

        double w_u_reg_running = 0.01;
        Eigen::Matrix<double, 7, 1> diag_u_reg_running;
        diag_u_reg_running << 1, 1, 1, 1, 1, 1, 10;
        ...
        for (int i = 0; i < _config.T; i++)
        {
            ...
            runningCostModel.get()->addCost(goal_cost_name_, frame_goal_cost, 0.0); // TODO: weight schedule
        ...
        terminalCostModel.get()->addCost(goal_cost_name_, frame_goal_cost, w_frame_terminal);
